...
export default class projectID extends Component {
  ...
  render() {
    return (
      <div className="m-2 bg-white font-bold flex  flex-col p-4">
        ...
        ></div>
        <div>Comment</div>
        <div className="border-2 rounded-lg p-2">{this.showCommentAll}</div>
        <div className="">
          <form onSubmit={this.onSubmitAction}>
            ...
            <input type="submit" value="Submit" className="p-2 mt-2 rounded-lg" />
          </form>
          <Feedback />
        </div>
      </div>
...
class Feedback extends Component {
  ...
  render() {
    return <div>{this.showLenghtOfList()}</div>;
  }
}
